        ...
        <div class="flex space-x-2">
            ...
            <div class="relative group">
                ...
                <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block">
                    ...
                    <div class="w-3 h-3 bg-black transform rotate-45 absolute -bottom-1 left-1/2 -translate-x-1/2">
                    </div>
                </div>
            </div>
            ...
            @if ($serviceRequest->status === 'PENDIENTE')
                <button onclick="serviceRequestModals.open('accept')"
                    class="bg-blue-500 hover:bg-blue-400 px-4 py-2 rounded">
            ...
            @if ($serviceRequest->status === 'ACEPTADA')
                @if ($serviceRequest->assigned_to)
                    <form action="{{ route('service-requests.start', $serviceRequest) }}" method="POST">
                        @csrf
                ...
                @else
                    <button type="button" class="btn btn-secondary" disabled title="Se requiere técnico asignado">
                        <i class="fas fa-play-circle mr-2"></i>
                        Iniciar Procesamiento (Requiere Asignación)
                    </button>
                    <p class="text-sm text-red-600 mt-1">
                        <i class="fas fa-exclamation-triangle mr-1"></i>
                        Asigna un técnico antes de iniciar el procesamiento
                    </p>
                @endif
            @endif
            ...
            @if (in_array($serviceRequest->status, ['ACEPTADA', 'EN_PROCESO']) && !$serviceRequest->is_paused)
                <button onclick="serviceRequestModals.open('pause')"
                    class="bg-orange-500 hover:bg-orange-400 px-4 py-2 rounded">
            ...
            @if (strtoupper(trim($serviceRequest->status)) === 'RESUELTA')
                <button onclick="serviceRequestModals.open('close')"
            ...
            @if (in_array(strtoupper(trim($serviceRequest->status)), ['PENDIENTE', 'ACEPTADA']))
                <button onclick="serviceRequestModals.open('cancel')"
                    class="bg-red-500 hover:bg-red-400 px-4 py-2 rounded">
