...
@php
    ...
    $evidencesSummary = $viewService->getEvidencesSummary($request);
    $resolveData = $viewService->getResolveButtonData($request);
@endphp
...
<div class="p-6 border-b">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('service-requests.evidences.create', $request) }}"
    ...
    <div class="bg-gray-50 rounded-lg p-4">
        ...
        <!-- Lista de Evidencias -->
        <div class="space-y-3">
            @foreach($request->evidences->sortBy('step_number') as $evidence)
            <div class="bg-white border rounded-lg p-4 hover:shadow-md transition-shadow">
                <div class="flex justify-between items-start">
                    ...
                    <div class="flex space-x-2 ml-4">
                        <a href="{{ route('service-requests.evidences.show', [$request, $evidence]) }}"
                            class="text-blue-600 hover:text-blue-800" title="Ver detalle">
                            <i class="fas fa-eye"></i>
                        </a>

                        @if($evidence->hasFile())
                        <a href="{{ route('service-requests.evidences.download', [$request, $evidence]) }}"
                        ...
                        @if(in_array($request->status, ['ACEPTADA', 'EN_PROCESO']))
                        <form action="{{ route('service-requests.evidences.destroy', [$request, $evidence]) }}"
