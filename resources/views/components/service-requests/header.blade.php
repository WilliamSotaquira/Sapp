...
<div class="bg-blue-600 text-white px-6 py-4">
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold">Solicitud: {{ $request->ticket_number }}</h2>
            <p class="text-blue-100">{{ $request->title }}</p>
        </div>
        ...
        <div class="flex space-x-2">
            ...
            <form action="{{ route('service-requests.start', $request) }}" method="POST" class="inline">
                @csrf
            ...
            <!-- PAUSAR -->
            @if(in_array($request->status, ['ACEPTADA', 'EN_PROCESO']) && !$request->is_paused)
            ...
            <!-- REANUDAR -->
            @if($request->isPaused())
            <form action="{{ route('service-requests.resume', $request) }}" method="POST" class="inline">
                @csrf
            ...
            <!-- RESOLVER -->
            @if($viewService->canShowResolveButton($request))
            @php $resolveData = $viewService->getResolveButtonData($request); @endphp
            @if($resolveData['can_resolve'])
            <a href="{{ route('service-requests.resolve-form', $request) }}"
            ...
            @endif
            @elseif($request->status === 'RESUELTA')
            ...
            <button class="bg-gray-300 px-4 py-2 rounded cursor-not-allowed"
                title="La solicitud debe estar en proceso para resolver (Estado actual: {{ $request->status }})">
                <i class="fas fa-check-circle mr-2"></i>Resolver
            </button>
            @endif

            <!-- CERRAR -->
            @if(strtoupper(trim($request->status)) === 'RESUELTA')
            ...
            <!-- CANCELAR -->
            @if(in_array(strtoupper(trim($request->status)), ['PENDIENTE', 'ACEPTADA']))
