            ...
            <form action="{{ route('services.store') }}" method="POST" class="p-6">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="code" id="code" maxlength="10"
                           ...
                           value="{{ old('code') }}"
                           placeholder="Ej: AP, DB, RC"
                           required>
                    <p class="mt-1 text-sm text-gray-500">
                        Código único de 2-10 caracteres para identificar el servicio
                    </p>
                    @error('code')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Describa el servicio...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="order" id="order"
                           ...
                           min="0">
                    <p class="mt-1 text-sm text-gray-500">
                        Número para ordenar los servicios (menor = primero)
                    </p>
                    @error('order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    <div class="flex items-center">
                        <input type="hidden" name="is_active" value="0"> <!-- Valor por defecto -->
                    ...
                    <p class="mt-1 text-sm text-gray-500">
                        Cuando está desactivado, el servicio no estará disponible para nuevas solicitudes
                    </p>
                    @error('is_active')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
...
<script>
// Asegurar que el checkbox envíe valores booleanos correctos
document.addEventListener('DOMContentLoaded', function() {
    const checkbox = document.getElementById('is_active');
    checkbox.addEventListener('change', function() {
        // El valor ya está configurado como 1/0 en el HTML
    });
});
</script>
@endsection
