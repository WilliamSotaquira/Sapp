...
class ServiceSubserviceSeederFixed extends Seeder
{
    public function run()
    {
        ...
        // Verificar que hay familias de servicio
        $families = ServiceFamily::all();

        if ($families->isEmpty()) {
            $this->command->error('No hay familias de servicio. Ejecuta primero ServiceFamilySeederManual.');
            return;
        }

        $this->command->info("Encontradas {$families->count()} familias de servicio");
        ...
        foreach ($families as $family) {
            $this->command->info("Procesando familia: {$family->name}");
            ...
            if (isset($subservicesData[$family->name])) {
                foreach ($subservicesData[$family->name] as $subserviceData) {
                    ...
                    $createdCount++;
                    $this->command->info("  - Creado: {$subserviceData['name']}");
                }
            } else {
                // Crear algunos subservicios genéricos para familias sin datos específicos
                ServiceSubservice::create([
                    'service_family_id' => $family->id,
                    'name' => "Subservicio Principal - {$family->name}",
                    'description' => "Subservicio principal para la familia {$family->name}",
                    'is_active' => true
                ]);
                $createdCount++;
                $this->command->info("  - Creado subservicio genérico para {$family->name}");
            }
        }

        $this->command->info("✅ {$createdCount} subservicios creados exitosamente!");
    }
}
